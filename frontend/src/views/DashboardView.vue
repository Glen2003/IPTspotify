        ...
        <div class="w-4/6 p-20   ">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
            <div v-for="track in tracks" :key="track.id" class="bg-gray-800 rounded-lg overflow-hidden shadow-lg">
              <iframe :src="generateEmbedUrl(track.id)" width="100%" height="250" frameborder="" allowfullscreen="" allow="clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>
              <div class="p-3">
...
const showSearch = ref(false); // New flag for search input visibility
...
// Watch for changes in searchQuery and trigger search automatically
watch(searchQuery, (newValue) => {
  if (newValue.trim() !== '') {
    search(newValue);
  } else {
    // Clear tracks if search query is empty
    tracks.value = [];
  }
});
</script>
